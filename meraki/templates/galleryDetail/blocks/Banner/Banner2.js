...
const Banner2 = ({
  ...
  return (
    <div className=" min-h-screen -mt-header  flex flex-col justify-center items-center w-full relative bg-element-9">
      <div className="flex relative w-full">
        ...
        <div className="py-header flex items-center flex-1">
          <Container>
            ...
            <h1 className="leading-none text-3xl  md:text-5xl font-kinfolk pt-6 max-w-sm">
              {title}
            </h1>
            <ul className="py-12 xl:text-lg space-y-3">
              {details.map((item, i) => (
                <li key={i} className="flex truncate flex-wrap items-baseline ">
                  <div className="font-extrabold mr-2">{item.label}</div>
                  <div>{item.value}</div>
                </li>
              ))}
            </ul>
