...
const Banner5 = ({ image = {}, title, subTitle, details = [] }) => {
  return (
    <div className=" min-h-screen relative -mt-header  flex w-full justify-center bg-element-9">
        ...
        <div className="py-header flex items-center w-full max-w-6xl">
          <div>
            <Container>
              ...
              <h1 className="leading-none text-3xl  md:text-4xl xl:text-5xl font-kinfolk pt-6 max-w-sm">
                {title}
              </h1>
              <ul className="py-12 mt-12 md:text-lg space-y-3">
                {details.map((item, i) => (
                  <li className="flex items-baseline space-x-2" key={i}>
