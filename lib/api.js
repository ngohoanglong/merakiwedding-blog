...
export async function getPostAndMorePosts(
  ...
  const postPreview =
    preview && previewData?.post
  // The slug may be the id of an unpublished post
  const isId = Number.isInteger(
    Number(slug)
  )
  ...
  const isDraft =
    isSamePost &&
    postPreview?.status === 'draft'
  const isRevision =
    isSamePost &&
    postPreview?.status === 'publish'
  const data = await fetchAPI(
    `
  ...
  if (
    ...
    const revision =
      data.post.revisions.edges[0]?.node
